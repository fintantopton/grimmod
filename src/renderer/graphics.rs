...
/// Linux/Windows use the per-frame hook/unhook pattern in surface_upload,
/// render_scene, etc., so no persistent hooks are installed at startup.
...
/// Hooks texture uploads to swap out regular assets for their HQ versions.
///
/// Uses per-frame hook/unhook of glTexImage2D and glPixelStorei.
...
///
/// Uses per-frame hook/unhook of sampler_parameteri.
